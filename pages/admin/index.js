        ...
        <header className="bg-white border-b border-gray-200 sticky top-0 z-40">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <Link href="/" className="font-bold text-gray-900 hover:text-brand-600 transition-colors">{BRAND_NAME}</Link>
            ...
            <>
              ...
              <div className="relative mb-4 max-w-sm">
                ...
                <input
                  ...
                  className="w-full border border-gray-200 rounded-lg pl-9 pr-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent"
                />
                {pesquisaAdmin && (
                  <button onClick={() => setPesquisaAdmin("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-300 hover:text-gray-500">×</button>
                )}
              </div>
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-gray-100 text-left text-xs text-gray-400 uppercase tracking-wider">
                          <th className="px-4 py-3 cursor-pointer select-none hover:text-gray-600" onClick={() => toggleOrdem("nome")}>
                            Produto <Seta col="nome" />
                          </th>
                          <th className="px-4 py-3 text-center cursor-pointer select-none hover:text-gray-600" onClick={() => toggleOrdem("sku")}>
                            SKU <Seta col="sku" />
                          </th>
                          ...
                          <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-3">
                                ...
                                <a href={`/produto/${p.id}`} target="_blank" rel="noopener noreferrer" className="font-medium text-gray-900 hover:text-brand-600 transition-colors line-clamp-1">{p.nome}</a>
                              </div>
                            </td>
                            <td className="px-4 py-3 text-gray-500 text-center">{p.sku || "—"}</td>
                            <td className="px-4 py-3 text-center">
                              {p.categoria ? (
                                <span className="bg-gray-100 text-gray-700 text-xs px-2 py-1 rounded-full">{p.categoria}</span>
                              ) : "—"}
                            </td>
                            <td className="px-4 py-3 text-gray-500 text-center">{p.imagens?.length || 0}</td>
                            <td className="px-4 py-3 text-center">
