// Cartão de produto — usado no catálogo e homepage
  ...
  return (
    <Link href={`/produto/${id}`} className="card group hover:shadow-md transition-shadow duration-200 flex flex-col">
      ...
      <div className={`relative bg-gray-100 overflow-hidden ${compact ? "aspect-[4/3]" : "aspect-square"}`}>
        ...
        {/* Badge categoria */}
        {categoria && (
          <span className="absolute top-3 left-3 bg-white/90 backdrop-blur-sm text-xs font-semibold text-brand-700 px-2.5 py-1 rounded-full">
            {categoria}
          </span>
        )}
      </div>
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        {descricaoCurta && (
          <p className="text-xs text-gray-500 leading-relaxed line-clamp-2 mt-auto pt-2">
            {descricaoCurta}
          </p>
        )}
        <div className="mt-3 flex items-center text-brand-600 text-xs font-semibold">
          Ver detalhes
